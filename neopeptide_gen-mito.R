...
bed = read.table('Genome_Canonical_Mito.bed', header = FALSE, sep = '\t')
...
for(i in 1:length(unq_geneIDs)){
  ...
  if(!isEmpty(forwardStrandPAMs)){
    for(k in 1:length(forwardStrandPAMs)){
      ...
      firstStop = min(c(which(codons(minusOne) == DNAString("AGA")), which(codons(minusOne) == DNAString("TAA")), which(codons(minusOne) == DNAString("TAG")), which(codons(minusOne) == DNAString("AGG"))))
      
      if(is.infinite(firstStop)){
        #there's no stop codon in the transcript -> non-stop decay! shouldn't see a protein in this scenario
      } else{
        #start = start(temp)-21+offset
        start = max(start(temp)-21+offset, 1)
        end = start(temp)-2+offset
        m1start = start(temp)-1
        m1pep = Biostrings::translate(minusOne[(m1start - shift[(m1start %% 3)+1]):(firstStop*3-3)],genetic.code=SGC1)
        ...
        if(length(m1pep)<8){
          ...
          write.fasta(sequences = list(m1pep),names = c(name1),open = "a",file.out = "frameshiftPeptidesMito.fasta")
      ...
      secondStop = min(c(which(codons(minusTwo) == DNAString("AGA")), which(codons(minusTwo) == DNAString("TAA")), which(codons(minusTwo) == DNAString("TAG")), which(codons(minusTwo) == DNAString("AGG"))))
      if(is.infinite(secondStop)){
        ...
        m2pep = Biostrings::translate(minusTwo[(m2start - shift[(m2start %% 3)+1]):(secondStop*3-3)],genetic.code=SGC1)
        ...
        if(length(m2pep)<8){
          ...
          write.fasta(sequences = list(m2pep),names = c(name2),open = "a",file.out = "frameshiftPeptidesMito.fasta")
  ...
  if(!isEmpty(reverseStrandPAMs)){
    for(l in 1:length(reverseStrandPAMs)){
      ...
      if(is.infinite(firstStop)){
        ...
        m1start = start(temp)
        m1pep = Biostrings::translate(minusOne[(m1start + shift_rev[(m1start %% 3)+1]):(firstStop*3-3)],genetic.code=SGC1)
      ...
      if(is.infinite(secondStop)){
        ...
        m2start = start(temp)
        m2pep = Biostrings::translate(minusTwo[(m2start + shift_rev[(m2start %% 3)+1]):(secondStop*3-3)],genetic.code=SGC1)
